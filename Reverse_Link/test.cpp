...
void CreateListTail(LNode *L, int n)
{
	LNode* p, *r;
	int i;                 
	L = (LNode*)malloc(sizeof(LNode)); /* L为整个线性表 */
	r = L;                                /* r为指向尾部的结点 */
	for (i = 0; i < n; i++)
	{
...
void Reverse_Iteration(LNode*L) //采用迭代头插法逆转链表
{
	if (L->next)//若原链表为空
	{
		return;
	}
	LNode* tmp = nullptr;//中间临时变量，用于存放链表节点
	LNode* header;//指向链表第一个节点
	header = (L)->next;
...
LNode* Reverse_Recursion(LNode*L)//采用递归头插法逆转链表
{
	if (!(L)->next)
	{
		return (L);
	...
	//递归翻转子列表
	LNode* newList = Reverse_Recursion((L)->next);//指向最后一个节点，在递归返回过程中一直指向最后一个节点
